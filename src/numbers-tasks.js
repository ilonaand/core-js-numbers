...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const theta = Math.acos(num / denum);
  return theta;
}
...
function isPrime(n) {
  let result = true;
  if (n <= 3) return true;
  for (let i = 2, s = Math.sqrt(n) + 1; i < s; i += 1) {
    result = n % i !== 0;
    if (!result) break;
  }
  return result;
...
function getSumOfDigits(num) {
  ...
  let sum = 0;
  const arr = [...str].map((i) => parseInt(i, 10));
...
function getCountOfOddNumbers(number) {
  const p = number < 0 ? Math.floor(number / 2) : Math.ceil(number / 2);
  return Math.abs(p);
}
